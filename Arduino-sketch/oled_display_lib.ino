/*
  ...
  |                 128 x 64 pixels, using IIC (I2C) connection to ATSAMD21 MCU.
  |
  |  Platform:      This version customized to suit SH1106 OLED controller.
  |                 SAMD21 library is needed for the "Wire" class IIC driver functions.
  |
...
// "Private" functions -- not meant to be called directly from the application
...
/*`````````````````````````````````````````````````````````````````````````````````````````````````
  ...
  |
  |    Function Disp_PutDigit_16p() references this table.
  |
  |  <!> This font Copyright by M.J.Bauer (2016++).  Permission is granted to use freely,
  |      on condition that M.J.Bauer is acknowledged as the font designer (www.mjbauer.biz).
*/
...
*/
const  uint8_t  font_table_digit_12x20_bold[] =
{
  // '0'
...
{
  if (FontSize >= 16)  Disp_PutChar16(uc);
...
/*----------------------------------------------------------------------------------------
  |  Name         :  Disp_PutDigit_16p()
  |
  |  Function     :  Display decimal digit in font size 16 (10 x 16 pix)
  |                  using a special smooth "high definition" font table.
  |                  On exit, CursorPosX is advanced 12 pixels
  |
  |  Input        :  digit = decimal value (0..9) to be displayed
  |  Return       :  --
    +--------------------------------------------------------------------------------------*/
void   Disp_PutDigit_16p(uint8_t digit)
{
  uint8_t  *bitmap;

  bitmap = (uint8_t *) &font_table_digit_10x16[digit * 32];
...
/*-------------------------------------------------------------------------------------
 |  Name          :  Disp_PutDigit_20p()
 |
 |  Function      :  Show decimal digit in font size 20 (12 x 20 pix)
 ...
 | Return         :  --
*/
void  Disp_PutDigit_20p(uint8_t digit)
{
  uint8_t  *bitmap; 
	
  bitmap = (uint8_t *) &font_table_digit_12x20_bold[digit * 40];
  Disp_PutImage(bitmap, 12, 20);
  CursorPosX += 16;
}
...
*
*--------------------------------------------------------------------------------------*/
void  Disp_PutDecimal(uint16_t uwVal, uint8_t bFieldSize)
{
  uint8_t   digit[6];     // digit[0] is LSD
  uint8_t   place, dig;
  ...
  for ( place = 4;  place < 5;  place-- )  // begin output with MSD
  {
    ...
    if (place < bFieldSize)  Disp_PutDigit(dig);  // Inside minimum field... output always,
    else if (!isLeading0)  Disp_PutDigit(dig);  // else... output only if NOT leading zero
  }
}
...
void  SH1106_WriteCommand(uint8_t cmd)
{
  ...
  Wire.write(SH1106_MESSAGETYPE_COMMAND);
  Wire.write(cmd);
...
{
  Wire.beginTransmission(SH1106_I2C_ADDRESS);
  Wire.write(SH1106_MESSAGETYPE_DATA);
  Wire.write(data);
  Wire.endTransmission();
}
  ...
  for (page = firstPage;  page <= lastPage;  page++)
  {
    ...
    for (col = firstCol;  col <= lastCol && col < 8;  col++)
    {
      ...
      for (x_coord = 0;  x_coord < 16;  x_coord++)
      {
        ...
        Wire.write(pixels);  // segment address auto increments
      }
      Wire.endTransmission();
    }
  }
