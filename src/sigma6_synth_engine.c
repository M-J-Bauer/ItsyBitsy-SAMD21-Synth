 ...
 * ================================================================================================
 *
 * Module:       sigma6_synth_engine.c
 *
 * Overview:     Sigma-6 sound synthesizer implementation.
 *
...
/***
        ...
        cents = g_Patch.OscDetune[osc];   // signed int (+/-600)
        detuneNorm = (IntToFixedPt(1) * cents) / 1200;  // normalized
        oscDetune = FixedToFloat(Base2Exp(detuneNorm));
        oscFreq = oscFreq * oscDetune;   // Apply OSC detune factor
***/
        // If oscFreq > 16 kHz, set flag -- Osc will be muted to avoid aliasing
        if (oscFreq > 16000.0)  m_OscMuted[osc] = TRUE;
...
void   TransientEnvelopeGen()
{
    ...
    {
        if (++envPhaseTimer >= 50)  // ENV2 Peak-Hold time = 50ms (fixed)
        {
...
void   AudioLevelController()
{
    ...
    fixed_t  exprnLevel;
    uint8   controlSource = g_Patch.AmpldControlSource;  // default
    ...
    else  controlSource = g_Patch.AmpldControlSource;

    if (controlSource == AMPLD_CTRL_CONST_LOW)  // mode 1
    {
        if (m_ENV1_Output)  outputAmpld = FIXED_MAX_LEVEL / 2;
        else  outputAmpld = 0;
    }
    ...
    {
        if (m_ENV1_Output)  outputAmpld = FIXED_MAX_LEVEL;  
        else  outputAmpld = 0;
    }

    if (outputAmpld > FIXED_MAX_LEVEL)  outputAmpld = FIXED_MAX_LEVEL;

    TIMER2_IRQ_DISABLE();  // because macro must not be disrupted...
    v_OutputLevel = FractionPart(outputAmpld, 10);  // unit = 1/1024, range 0..1023
    TIMER2_IRQ_ENABLE();

    // Check audio output for clipping...
    if (v_Clipping)
    {
        v_Clipping = 0;
        CLIPPING_LED_ON();
    ...
    if (clipLEDstate == 1)
    {
        if (++clipLEDduty_ms >= 100)  { CLIPPING_LED_OFF();  clipLEDstate = 0; }
    }
    else  clipLEDduty_ms = 0;
}
 ...
 * an unsigned 8-bit value = LFO freq * 10 H, range 1..250 => 0.1 to 25 Hz.
 * See also 'OscFreqModulation()' which updates the input variable m_LFO_Step.
...
void   VibratoRampGenerator()
{
    ...
    if (rampState != 3 && (!m_Note_ON || m_LegatoNoteChange))
    {
...
void   OscFreqModulation()
{
    ...
    int32   oscFreqLFO;      // 24:8 bit fixed-point format
    short   osc, cents;
 ...
 *                 (These are scalar multipliers, range 0..1000)
 */
void  OscAmpldModulation()
{
    short  osc, step;
    ...
    // Update Mixer Output Gain control according to patch param.
    v_MixerOutGain = (g_Patch.MixerOutGain_x10 << 7) / 100;  // = MixerOutputGain x (128/100)
}
...
void  __ISR(_TIMER_2_VECTOR, IPL6AUTO)  Timer_2_IRQService(void)
{
    ...
    READ_CPU_CORE_COUNT_REG(CC_Reg);  // to measure ISR execution time (duty)
    entryTime = CC_Reg;
    ...
    if (v_SynthEnable)
    {
        ...
        mixerOut = (mixerOut * v_MixerOutGain) >> 7;  // = mixerOut * MixerOutGain / 128
        
        if (mixerOut > FIXED_MAX_LEVEL)  // Apply limiter
        {
            mixerOut = FIXED_MAX_LEVEL;
            v_Clipping = TRUE;  // Trigger LED indicator
        }
        if (mixerOut < (0 - FIXED_MAX_LEVEL))  mixerOut = 0 - FIXED_MAX_LEVEL;

        // Variable-gain output attenuator -- Apply envelope, velocity, expression, etc.
    ...
    // PWM DAC output... Update OC4 duty register (11 bits, range 1..1999)
    PWM_AUDIO_DAC_WRITE(1000 + (int)(finalOutput >> 10));
    
#ifdef __32MX440F256H__  // MAM board + Pinguino-micro + SPI DAC
    spiDACdata = (uint16)(2000 + (int)(finalOutput >> 9));  // 12 bits
    SPI_Lock(2);
    SPI_DAC_CS_LOW();
 ...
 *
 * Overview:  This ISR handles any task(s) which must be executed synchronously with  
 *            the Timer_3 period rollover... Nothing to do in this application!
 */
